            ...
            foreach (ISingleCrack singleCrack in crack.SingleCracks)
            {
                ...
                foreach (IXFiniteElement element in tipEnrichedElements)
                {
                    if (element.Subdomain != tipSubdomain)
                    {
                        ...
                        //      binary search.
                        element.Subdomain.Elements.Remove(element.ID);
                        modifiedSubdomains.Add(element.Subdomain);

                        //TODO: It should be added to a position that will not affect the bandwidth too adversely.
                        tipSubdomain.Elements.Add(element.ID, element);
