...
namespace ISAAR.MSolve.MultiscaleAnalysis
{
    ...
    /// </summary>
    public class HomogeneousRVEBuilderNonLinearNoRenum : IRVEbuilder
    {        
        public HomogeneousRVEBuilderNonLinearNoRenum()
        {
            //TODOGerasimos
            // this.renumbering_vector_path=renumbering_vector_path,
            // this.subdiscr1=subdiscr1
        }
        public IRVEbuilder Clone(int a) => new HomogeneousRVEBuilderNonLinearNoRenum();
        ...
        public static Tuple<Model, Dictionary<int, Node>,double> Reference2RVEExample10_000withRenumbering_mono_hexa()
        {
            ...
            int total_nodes = (1 + hexa1) * (1 + hexa2) * (1 + hexa3);
            int[] totalNodesNumbering = new int[total_nodes];
            for (int i1 = 0; i1 < total_nodes; i1++)
            {
                totalNodesNumbering[i1] = i1 + 1;
            }
            renumbering renumbering = new renumbering(totalNodesNumbering);
