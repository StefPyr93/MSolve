...
{
    public static class SeparateCodeCheckingClass5b_bNEW_debugGitTestPRINT
    {//ORIGIN: SeparateCodeCheckingClass5b_bNEW_debugGitTest
     //Changes: uses sparse solver, prints reunumbering also, sets appropriate cnstvalues san na etrexe analyzer.(iters)
        [Fact]
        static void Solve()
        {
            for (int example = 28; example < 29; example++)
            {
                ...
                (Vector globalU1_2, int[] serialModelFromsubstructuredTade) = ReorderDirectSolverSolutionIn_globalU1_format(globalU1, globalU2, model1, model2);

                string print_path_gen = (new CnstValues()).exampleOutputPathGen + @"\subdomain_matrices_and_data\serialModelDofOrderFromsubstructuredTade.txt";
                PrintUtilities.WriteToFileVector(serialModelFromsubstructuredTade, print_path_gen);
        ...
        private static (Vector, int[]) ReorderDirectSolverSolutionIn_globalU1_format(Vector globalU1, Vector globalU2, Model model1, Model model2)
        {
            ...
            int[] serialModelFromsubstructuredTade = new int[globalU1.Length];
            foreach (int nodeID in freeNodesIds)
            {
                foreach (var dof in model2.GlobalDofOrdering.GlobalFreeDofs.GetDataOfRow(model2.GetNode(nodeID)).Keys)
                {
                    ...
                    serialModelFromsubstructuredTade[model1Dof_order] = model2Dof_order;
                }
            }

            return (globalU1_2, serialModelFromsubstructuredTade);
        }
