...
namespace ISAAR.MSolve.LinearAlgebra.Matrices
{
    ...
    public class Matrix : IMatrix, ISliceable2D, IEntrywiseOperableView2D<Matrix, Matrix>, IEntrywiseOperable2D<Matrix>
    {
        ...
        public static Matrix CreateDiagonalFromVector(Vector vector)
        {
            int order = vector.Length;
            double[] data = new double[order * order];
            for (int j = 0; j < order; ++j) data[j * order + j] = vector[j];
            return new Matrix(data, order, order);
        }
        ...
        public void SetDiagonal(Vector diagonalValues)
        {
            if (this.NumRows != this.NumColumns) throw new NonMatchingDimensionsException(
                "Matrix is not square");
            if (diagonalValues.Length > this.NumRows) throw new NonMatchingDimensionsException(
                "The entries to set exceed this square matrix's dimension");
            for (int i=0; i<this.NumRows; i++)
            {
                this.data[i * this.NumRows + i] = diagonalValues[i];
            }
        }
