...
namespace ISAAR.MSolve.XFEM.Tests.COMPDYN2019
{
    public static class Utilities
    {
        ...
        public static Dictionary<ISubdomain, HashSet<INode>> FindCornerNodesFromCrosspoints2D(IModel model)
        {
            ...
            foreach (ISubdomain subdomain in model.EnumerateSubdomains())
            {
                ...
                cornerNodes[subdomain] = new HashSet<INode>(subdomain.EnumerateNodes().Where(
                    node => node.SubdomainsDictionary.Count > 2));
            }
            return cornerNodes;
        }

        public static Dictionary<ISubdomain, HashSet<INode>> FindCornerNodesFromRectangleCorners(IModel model)
        {
            var cornerNodes = new Dictionary<ISubdomain, HashSet<INode>>();
            foreach (ISubdomain subdomain in model.EnumerateSubdomains())
            {
                INode[] subdomainCorners = CornerNodeUtilities.FindCornersOfRectangle2D(subdomain);
                cornerNodes[subdomain] = new HashSet<INode>(subdomainCorners.Where(node => node.Constraints.Count == 0));
            }
            return cornerNodes;
        }
        ...
        public static double RunSingleCrackedStep(XModel model, ICrackDescription crack, ISolver solver)
        {
            ...
            else if (solver is FetiDPSolverOLD fetiDPSolver)
            {
                var sudomainDisplacements = new Dictionary<int, IVectorView>();
